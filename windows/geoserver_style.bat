...
FOR /r %%f in (%1\*.sld) do (
	...
	# layer
	curl -v -u %login% -XPOST -H "Content-type: text/xml" -d "<featureType><name>%%~nf</name></featureType>" %restapi%/workspaces/%workspace%/datastores/%store%/featuretypes?recalculate=nativebbox,latlonbbox
	
	rem style
	echo -v -u %login% -XPOST -H "Content-type: text/xml" -d "<style><name>%%~nf</name><filename>%%~nf%%~xf</filename></style>" %restapi%/workspaces/%workspace%/styles
